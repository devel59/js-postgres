...
describe('customization', () => {
    test('transaction instance', async () => {
        const db = new Database({
            logger: fakeLogger,
            pool: createRowPool(0),
            createTransactionDatabase({ pgClient, parentDatabase }) {
                return new CustomTransactionDatabase({
                    pgClient,
    ...
    test('task instance', async () => {
        const db = new Database({
            logger: fakeLogger,
            pool: createRowPool(0),
            createTaskDatabase({ pgClient, parentDatabase }) {
                return new CustomTaskDatabase({
                    pgClient,
                    logger: parentDatabase.logger
                });
            }
        ...
        const db = new Database({
            ...
            createTaskDatabase({ pgClient, parentDatabase }) {
                return new CustomTaskDatabase({
                    pgClient,
                    logger: parentDatabase.logger,
                    createTransactionDatabase: this.createTransactionDatabase
                });
            }
        });
